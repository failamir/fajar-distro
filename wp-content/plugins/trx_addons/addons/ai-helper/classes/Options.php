	...
	class Options {
			...
			trx_addons_array_insert_before( $options, 'users_section', apply_filters( 'trx_addons_filter_options_ai_helper', array(
				...
				*/
				...
				'ai_helper_default_api_stabble_diffusion' => array(
					...
					"std" => "v6",
					"options" => array(
						"v3" => esc_html__("V3 (Legacy)", 'trx_addons'),
						"v6" => esc_html__("V6 Realtime (New)", 'trx_addons'),
					),
					"type" => "radio",
					"dependency" => array(
						"ai_helper_token_stabble_diffusion" => array('not_empty'),
				...
				'ai_helper_text_model_default' => array(
					...
					"std" => "",
					"options" => apply_filters( 'trx_addons_filter_ai_helper_list_models', $is_options_page ? Lists::get_list_ai_text_models() : array(), 'text_model' ),
					"type" => "select",
				),
				// Leave name 'ai_helper_text_model_default' for compatibility with old versions
		...
		function fix_options( $options ) {
			...
			foreach ( array( '', '_premium' ) as $suffix ) {
				foreach ( array( 'ai_helper_sc_igenerator_limit_per_request', 'ai_helper_sc_chat_limit_per_request' ) as $option ) {
					$name = $option . $suffix;
		 ...
		 */
		 function remove_token_from_export( $options ) {
			...
			$reset_options = apply_filters( 'trx_addons_filter_ai_helper_options_to_reset', array(
				...
				'ai_helper_embed_chats' => array( 'default' => array(), 'filter' => '<script' ),	// Remove chats if embed code contains <script> tag
			) );
			// Remove log
					...
					if ( ! empty( $value['filter'] ) ) {
						if ( is_array( $options['trx_addons_options'][ $option ] ) ) {
							foreach( $options['trx_addons_options'][ $option ] as $k => $v ) {
								if ( is_string( $v ) && strpos( $v, $value['filter'] ) !== false ) {
