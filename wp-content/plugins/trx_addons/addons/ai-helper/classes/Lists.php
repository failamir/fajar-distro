...
if ( ! class_exists( 'Lists' ) ) {
	...
	class Lists {
		...
		static function get_list_ai_chat_models( $sc = 'sc_chat' ) {
			// Make a default list APIs for the compatibility with old installations
			...
			// Get an API order
			$api_order = trx_addons_get_option( "ai_helper_{$sc}_api_order", $api_list );
		...
		static function get_default_google_ai_chat_models() {
			return apply_filters( 'trx_addons_filter_ai_helper_google_ai_models', array(
				'gemini-pro' => array(
					...
					'max_tokens' => 32700,	// inputTokenLimit: 30720, outputTokenLimit: 2048
				),
				'gemini-pro-vision' => array(
					'id' => 'gemini-pro-vision',
					'title' => esc_html__( 'Gemini Pro Vision', 'trx_addons' ),
					'max_tokens' => 16350,	// inputTokenLimit: 12288, outputTokenLimit: 4096
				)
			) );
		}
