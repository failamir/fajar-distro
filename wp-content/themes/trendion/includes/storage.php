...
if ( ! function_exists( 'trendion_storage_get' ) ) {
	/**
	 ...
	 */
	function trendion_storage_get( $var_name, $default = '' ) {
		global $TRENDION_STORAGE;
		return isset( $TRENDION_STORAGE[ $var_name ] ) ? $TRENDION_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'trendion_storage_set' ) ) {
	/**
	 ...
	 */
	function trendion_storage_set( $var_name, $value ) {
		global $TRENDION_STORAGE;
		$TRENDION_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'trendion_storage_empty' ) ) {
	/**
	 ...
	 */
	function trendion_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $TRENDION_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $TRENDION_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $TRENDION_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $TRENDION_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'trendion_storage_isset' ) ) {
	/**
	 ...
	 */
	function trendion_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $TRENDION_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $TRENDION_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $TRENDION_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $TRENDION_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'trendion_storage_unset' ) ) {
	/**
	 ...
	 */
	function trendion_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $TRENDION_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $TRENDION_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $TRENDION_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $TRENDION_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'trendion_storage_inc' ) ) {
	/**
	 ...
	 */
	function trendion_storage_inc( $var_name, $value = 1 ) {
		global $TRENDION_STORAGE;
		if ( empty( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = 0;
		}
		$TRENDION_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'trendion_storage_concat' ) ) {
	/**
	 ...
	 */
	function trendion_storage_concat( $var_name, $value ) {
		global $TRENDION_STORAGE;
		if ( empty( $TRENDION_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'trendion_storage_get_array' ) ) {
	/**
	 ...
	 */
	function trendion_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $TRENDION_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $TRENDION_STORAGE[ $var_name ][ $key ] ) ? $TRENDION_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $TRENDION_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $TRENDION_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'trendion_storage_set_array' ) ) {
	/**
	 ...
	 */
	function trendion_storage_set_array( $var_name, $key, $value ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$TRENDION_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'trendion_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function trendion_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $TRENDION_STORAGE[ $var_name ][ $key ] ) ) {
			$TRENDION_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$TRENDION_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$TRENDION_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'trendion_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function trendion_storage_merge_array( $var_name, $key, $value ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$TRENDION_STORAGE[ $var_name ] = array_merge( $TRENDION_STORAGE[ $var_name ], $value );
		} else {
			$TRENDION_STORAGE[ $var_name ][ $key ] = array_merge( $TRENDION_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'trendion_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function trendion_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			trendion_array_insert_after( $TRENDION_STORAGE[ $var_name ], $after, $key );
		} else {
			trendion_array_insert_after( $TRENDION_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'trendion_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function trendion_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			trendion_array_insert_before( $TRENDION_STORAGE[ $var_name ], $before, $key );
		} else {
			trendion_array_insert_before( $TRENDION_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'trendion_storage_push_array' ) ) {
	/**
	 ...
	 */
	function trendion_storage_push_array( $var_name, $key, $value ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $TRENDION_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $TRENDION_STORAGE[ $var_name ][ $key ] ) ) {
				$TRENDION_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $TRENDION_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'trendion_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function trendion_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $TRENDION_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $TRENDION_STORAGE[ $var_name ] ) && is_array( $TRENDION_STORAGE[ $var_name ] ) && count( $TRENDION_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $TRENDION_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $TRENDION_STORAGE[ $var_name ][ $key ] ) && is_array( $TRENDION_STORAGE[ $var_name ][ $key ] ) && count( $TRENDION_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $TRENDION_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'trendion_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function trendion_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( empty( $TRENDION_STORAGE[ $var_name ][ $key ] ) ) {
			$TRENDION_STORAGE[ $var_name ][ $key ] = 0;
		}
		$TRENDION_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'trendion_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function trendion_storage_concat_array( $var_name, $key, $value ) {
		global $TRENDION_STORAGE;
		if ( ! isset( $TRENDION_STORAGE[ $var_name ] ) ) {
			$TRENDION_STORAGE[ $var_name ] = array();
		}
		if ( empty( $TRENDION_STORAGE[ $var_name ][ $key ] ) ) {
			$TRENDION_STORAGE[ $var_name ][ $key ] = '';
		}
		$TRENDION_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'trendion_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function trendion_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $TRENDION_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $TRENDION_STORAGE[ $var_name ] ) ? $TRENDION_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $TRENDION_STORAGE[ $var_name ] ) ? $TRENDION_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'trendion_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function trendion_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $TRENDION_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $TRENDION_STORAGE[ $var_name ]->$prop ) ? $TRENDION_STORAGE[ $var_name ]->$prop : $default;
	}
}
