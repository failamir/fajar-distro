...
jQuery( document ).ready(
	function() {
		...
		trendion_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="trendion_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.trendion_post_editor' );
						if ( link.length > 0 ) {
							trendion_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.trendion_post_editor', function(e) {
						trendion_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('trendion_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function trendion_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
