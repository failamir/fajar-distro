...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			trendion_create_observer( 'trendion-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				trendion_gutenberg_editor_init();
			} );
		} else {
		 	trendion_gutenberg_editor_init();
		}
	...
	function trendion_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'trendion_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'trendion_inited' ) ) {
	...
	// Init on page load
	function trendion_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = trendion_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + TRENDION_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + TRENDION_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in TRENDION_STORAGE['override_classes'] ) {
				$obj.addClass( TRENDION_STORAGE['override_classes'][i].replace( '%s', TRENDION_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( trendion_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && TRENDION_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'trendion_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			trendion_remove_observer( 'trendion-check-editor-styles-wrapper' );
			trendion_create_observer( 'trendion-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'trendion_inited' );
	}
} );
