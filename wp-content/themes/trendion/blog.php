...
if ( function_exists( 'trendion_elementor_is_preview' ) && trendion_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			trendion_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	trendion_new_main_query(
		array(
			'post_type'      => trendion_get_theme_option( 'post_type' ),
			'category'       => trendion_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'trendion_filter_get_template_part', trendion_blog_archive_get_template() ) );
}
