<div class="front_page_section front_page_section_team<?php
	$trendion_scheme = trendion_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $trendion_scheme ) && ! trendion_is_inherit( $trendion_scheme ) ) {
		echo ' scheme_' . esc_attr( $trendion_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( trendion_get_theme_option( 'front_page_team_paddings' ) );
	if ( trendion_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$trendion_css      = '';
		$trendion_bg_image = trendion_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$trendion_anchor_icon = trendion_get_theme_option( 'front_page_team_anchor_icon' );
	$trendion_anchor_text = trendion_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( trendion_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$trendion_css      = '';
			$trendion_bg_mask  = trendion_get_theme_option( 'front_page_team_bg_mask' );
			$trendion_bg_color_type = trendion_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $trendion_bg_color_type ) {
				$trendion_bg_color = trendion_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$trendion_caption = trendion_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $trendion_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $trendion_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $trendion_caption, 'trendion_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$trendion_description = trendion_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $trendion_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $trendion_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $trendion_description ), 'trendion_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! trendion_exists_trx_addons() ) {
						...
						trendion_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
