...
( function() {
	...
	***************************/
	// Widget Slider
	trendion_trx_addons_slider_init();

	// Shortcode Blogger
	trendion_trx_addons_blogger_init();

	// Menu
	trendion_trx_addons_menu_init();
	...
	***************************/
	$document.ready(function() {
		// Widget Recent News 
		trendion_trx_addons_recent_news_ready();

		// Widget Recent Posts 
		trendion_trx_addons_recent_posts_ready();

		// Widget Video List
		trendion_trx_addons_video_list_ready();

		// Menu
		trendion_trx_addons_menu_ready();

		// Layouts
		trendion_trx_addons_layouts_ready();

		// Register/login tabs
		trendion_trx_addons_login_ready();

		// Widget Rating Posts
		trendion_trx_addons_rating_posts_ready();

		// Shortcode Blogger: Images
		trendion_trx_addons_blogger_images_ready_ajax();

		// Widget Slider
		trendion_trx_addons_slider_ready();
	});
	...
	***************************/
	$window.load(function() {
		...
		trendion_trx_addons_blogger_slider_load_resize();

		// Widget Slider
		trendion_trx_addons_slider_load_resize_ajax();
	});
	...
	***************************/
	$window.resize(function() {
		...
		// Widget Video list
		trendion_trx_addons_video_list_resize();
	});
	...
	***************************/
	...
	/* AJAX
	***************************/
	$document.on( 'action.ajax_trendion', function() { 
		// Shortcode Blogger: Images
		trendion_trx_addons_blogger_images_ready_ajax();
	...
	***************************/
	...
	// Widget Slider
	function trendion_trx_addons_slider_init() {
		// Slider min width
		trendion_add_filter('trx_addons_filter_slider_controller_slide_width', function(width) {
			return 140;
		});

		// Slider all params
		trendion_add_filter('trx_addons_filter_slider_init_args', function(params, slider) {
		...
		trendion_add_filter('trx_addons_filter_slider_controls_html_fraction', function(html, controls) {
	...
	// Shortcode Blogger
	function trendion_trx_addons_blogger_init() {
		trendion_add_filter('trx_addons_filter_set_min_height_on_switch_tabs', function(set, sc) {
	...
	// Menu
	function trendion_trx_addons_menu_init() {
		// Superfish menu params
		trendion_add_filter('trx_addons_filter_menu_init_args', function(sf_init) {
		...
		trendion_add_action('trx_addons_action_menu_on_before_show', function(menu_item) {
		...
		trendion_add_filter('trx_addons_filter_menu_animation_in', function(animated, menu_item, animation_in, animation_out) {		
			var delay = 0;
			if ( !is_touch_device ) {
		...
		trendion_add_action('trx_addons_action_menu_before_animation_out', function() {
			clearTimeout(window.menuFadeTimer);
		});

		// Timer for fix/unfix row
		trendion_add_filter('trx_addons_filter_sc_layouts_row_fixed_off_timeout', function(time) {		
			return 200;
		});	

		// Stretch sumbenu
		trendion_add_filter('trx_addons_filter_stretch_menu_to', function(result, menu_item) {
	...
	// Widget Recent News
	function trendion_trx_addons_recent_news_ready() {
	...
	// Widget Recent Posts
	function trendion_trx_addons_recent_posts_ready() {
	...
	// Widget Video List 
	function trendion_trx_addons_video_list_ready() {
	...
	// Menu 
	function trendion_trx_addons_menu_ready() {
		if ( jQuery('.elementor-nav-menu').length > 0 ) { 
			jQuery('.elementor-nav-menu .sc_layouts_submenu').each(function() {
				...
				jQuery(this).addClass('sub-menu elementor-nav-menu--dropdown').removeClass('sc_layouts_submenu').append('<li class="menu-item"><span><a href="#" style="pointer-events: none;">' + TRENDION_STORAGE['submenu_not_allowed'] + '</a></span></li>');
			});
		}
	}		

	// Layouts
	function trendion_trx_addons_layouts_ready() {
	...
	// Register/login tabs
	function trendion_trx_addons_login_ready() {		
	...
	// Widget Rating Posts
	function trendion_trx_addons_rating_posts_ready() {
	...
	// Widget Slider
	function trendion_trx_addons_slider_ready() {
	...
	// Shortcode Blogger: Images
	function trendion_trx_addons_blogger_images_ready_ajax() {
		// Blogger Wide
	...
	// Shortcode Blogger: Slider
	function trendion_trx_addons_blogger_slider_load_resize() {
	...
	// Widget Slider
	function trendion_trx_addons_slider_load_resize_ajax() {
	...
	// Widget Video list
	function trendion_trx_addons_video_list_resize() {
