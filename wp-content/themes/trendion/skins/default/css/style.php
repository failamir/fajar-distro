...
if ( ! function_exists( 'trendion_customizer_get_css' ) ) {

	function trendion_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = trendion_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.trendion_demo_tabs_title {
	{$fonts['button_font-family']}
	{$fonts['button_font-weight']}
}
.theme-trendion .cky-btn {
	{$fonts['button_font-family']}
	{$fonts['button_text-transform']}
}
...
CSS;
			$rez          = apply_filters( 'trendion_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = trendion_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'trendion_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( trendion_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = trendion_get_scheme_colors( $s );
					} 

					// Make theme-specific colors and tints
					$colors         = trendion_customizer_add_theme_colors( $colors );
					$rez['colors']  = '';
					$rez            = apply_filters( 'trendion_filter_get_css', $rez, array( 'colors' => $colors, 'scheme' => $s ) ); 
		...
		return apply_filters( 'trendion_filter_prepare_css', $css_str, $remove_spaces );
	}
}
