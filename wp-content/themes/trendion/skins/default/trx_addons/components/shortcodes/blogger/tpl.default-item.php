...
if ( get_post_type() == 'product' ) {
	$args['hover'] = trendion_get_theme_option( 'shop_hover' );
}
		...
		?><div class="sc_blogger_item_body"<?php echo ( !in_array( $args['image_ratio'], array('', 'none', 'masonry') ) && !has_post_thumbnail() ? ' data-ratio="' . esc_attr($args['image_ratio']) . '"' : ''); ?>><?php
			...
			if ( isset($template['layout']['featured']) ) {
				...
				if ( $post_format == 'gallery' && in_array( $args['image_ratio'], array('', 'none', 'masonry') ) ) {
					...
					$x = $y = 0;
					if ( $args['columns'] > 2 ) {
						$x = $thumb_sizes['trendion-thumb-med']['size'][0];
						$y = $thumb_sizes['trendion-thumb-med']['size'][1];
					} else {
						$x = $thumb_sizes['trendion-thumb-big']['size'][0];
						$y = $thumb_sizes['trendion-thumb-big']['size'][1];
					}
					if ( $x != 0 && $y != 0 ) {
						$thumb_ratio = $x . ':' . $y;
					}
