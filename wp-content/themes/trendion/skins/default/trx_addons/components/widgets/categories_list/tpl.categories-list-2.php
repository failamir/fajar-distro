			...
			if ($show_thumbs) {
				...
				?><div class="categories_list_image"><?php
					$src = empty($image) 
								...
								: trx_addons_add_thumb_size($image, trendion_get_thumb_size('med-5'));
					$attr = trx_addons_getimagesize($src);
					?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category image', 'trendion'); ?>">
				</div><?php
			}
